...
uint8_t selectDisplay(uint8_t whichDisp)
{
    switch(whichDisp)
    {
        case 1: // Turn on first display
            SEL_DISPLAY_DDR = ((SEL_DISPLAY_DDR & MASK_PORTC) | FIRST_7SEG);
            break;
        case 2: // Turn on second display
...
uint8_t selectNumber(uint8_t whichNumber)
{
    ...
    {
        case 0:
            SEL_NUMBER_PORT = ZERO_7SEG; // mask is not necessary, because all
                               // SEL_NUMBER_PORT is for 7seg display purposes.
            break;
        case 1:
